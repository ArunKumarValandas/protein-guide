...
import { TREE_ALGO_LIST } from '@/constants/trees';
...
export function TreesPage() {
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-content">Trees</h1>
        <p className="mt-2 text-content-muted">
          Visualize Binary Search Tree operations and depth-first traversal orders.
        </p>
      </div>

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {TREE_ALGO_LIST.map((algorithm) => (
          <Link key={algorithm.id} to={`${ROUTES.TREES}/${algorithm.id}`}>
