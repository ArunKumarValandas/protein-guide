...
import { GREEDY_ALGORITHMS } from '@/constants/greedy';
...
export function GreedyVisualizerPage() {
  const { algorithm: algorithmId = 'activity-selection' } = useParams();
  const algorithm = GREEDY_ALGORITHMS[algorithmId] || GREEDY_ALGORITHMS['activity-selection'];
  ...
  const generateAndRun = useCallback(() => {
    const nextFrames = runGreedyAlgorithm(algorithm.id);
  ...
  usePlayback(() => {
    addRecentActivity({ type: 'greedy', algorithm: algorithm.id, name: algorithm.name });
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          <Link to={ROUTES.GREEDY}>
            <Button variant="ghost" size="icon" aria-label="Back to greedy algorithms">
      ...
      {activeTab === 'visualize' && (
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="space-y-6 lg:col-span-2">
            <div id="visualizer-export" className="card min-h-[340px]">
              <GreedyVisualizer visualData={visualData} highlights={highlights} />
            </div>
            <StatsPanel
              ...
              message={message}
              labels={{ comparisons: 'Choices', swaps: 'Selections' }}
            />
          </div>
          <div className="card">
            <PlaybackControls
              ...
              randomLabel="Reset greedy example"
            />
          </div>
        </div>
      )}
