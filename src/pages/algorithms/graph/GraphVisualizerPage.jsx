...
import { GRAPH_ALGORITHMS } from '@/constants/graph';
...
export function GraphVisualizerPage() {
  const { algorithm: algorithmId = 'bfs' } = useParams();
  const algorithm = GRAPH_ALGORITHMS[algorithmId] || GRAPH_ALGORITHMS.bfs;
  ...
  const generateAndRun = useCallback(() => {
    const nextFrames = runGraphAlgorithm(algorithm.id);
  ...
  usePlayback(() => {
    addRecentActivity({ type: 'graph', algorithm: algorithm.id, name: algorithm.name });
    updateProgress(algorithm.id, { completed: true });
    unlockAchievement('graph_explorer');
  });
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          <Link to={ROUTES.GRAPH}>
            <Button variant="ghost" size="icon" aria-label="Back to graph algorithms">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="space-y-6 lg:col-span-2">
            <div id="visualizer-export" className="card min-h-[380px]">
              <GraphVisualizer visualData={visualData} highlights={highlights} />
            </div>
            <StatsPanel
          ...
          <div className="space-y-4">
            <div className="card">
              <PlaybackControls
                ...
                randomLabel="Reset graph"
              />
            </div>
            <div className="card space-y-3 text-sm text-content-muted">
              <p>Start node is A. Edge weights are shown directly on the graph.</p>
              <p>Traversal, distance, and MST state update as each frame advances.</p>
            </div>
          </div>
        </div>
      )}
